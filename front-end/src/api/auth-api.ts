...
const fetchGoogleRedirectUri = async (): Promise<FetchGoogleRedirectUriAPIResponse> => {
  const response = await axiosInstance.get<FetchGoogleRedirectUriAPIResponse>(API_VARIABLES.AUTH_ENDPOINTS.FETCH_GOOGLE_REDIRECT_URI)
...
const exchangeCodeForToken = async (code: ExchangeCodeForTokenInput): Promise<ExchangeCodeForTokenAPIResponse> => {
