...
\subsubsubsection{Resultado dos testes}

O quadro abaixo (\ref{quad:resumo-testes}) ilustra todos os testes de integração feitos e seus respectivos resultados:


\begin{quadro}[htbp] % ou [ht] / [H] conforme necessidade de posição
	\centering
	\caption{Resumo dos Testes Executados}
	\label{quad:resumo-testes}
	\begin{tabularx}{\textwidth}{|X|p{3cm}|}
		\hline
		\textbf{Teste} & \textbf{Resultado} \\
		\hline
		Sobreposição de agendamentos & \textit{PASS} \\
		\hline
		Autenticação no sistema & \textit{PASS} \\
		\hline
		Restrição na antecedência de tempo ao agendar & \textit{PASS} \\
		\hline
		Filtro de serviços & \textit{PASS} \\
		\hline
		Controle de acesso por roles & \textit{PASS} \\
		\hline
	\end{tabularx}
	\fonte{Produzido pelos autores.}
\end{quadro}
...
O Apêndice \ref{app:integracao} deste documento possui capturas de tela que detalham o resultado de cada teste acima.
...
A pontuação A+ confirma que a aplicação segue rigorosamente as boas práticas de segurança recomendadas por organizações como \textit{OWASP} (\textit{Open Web Application Security Project}).

O Apêndice \ref{app:ssl} deste documento ilustra o resultado da análise \gls{ssl} de forma integral.
