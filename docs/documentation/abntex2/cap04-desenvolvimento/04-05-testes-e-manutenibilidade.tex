...
\paragraph{Teste de Segurança SSL/TLS (\textit{SSL Test})}
Além da validação dos cabeçalhos de segurança HTTP, a aplicação foi submetida a testes de configuração SSL/TLS utilizando a ferramenta \textbf{\textit{SSL Labs}}~\cite{ssllabs-2025}, desenvolvida pela Qualys. 
Esta ferramenta realiza uma análise abrangente da camada de transporte criptografada, avaliando a robustez da implementação HTTPS e identificando possíveis vulnerabilidades na configuração do servidor.

O teste SSL/TLS verifica aspectos críticos da segurança de transporte, incluindo:

\begin{itemize}
	\item \textbf{Versões de protocolo:} Validação das versões TLS suportadas (TLS 1.2, TLS 1.3) e detecção de protocolos obsoletos e inseguros (SSL 2.0, SSL 3.0, TLS 1.0, TLS 1.1).
	\item \textbf{\textit{Cipher Suites}:} Análise dos conjuntos de cifras habilitados, priorizando algoritmos modernos e seguros (\textit{ECDHE}, \textit{AES-GCM}) e identificando cifras fracas ou depreciadas.
	\item \textbf{Certificados digitais:} Verificação da validade, cadeia de confiança, algoritmos de assinatura e conformidade com padrões da indústria.
	\item \textbf{Vulnerabilidades conhecidas:} Detecção de falhas como \textit{POODLE}, \textit{BEAST}, \textit{Heartbleed}, \textit{CRIME}, \textit{BREACH} e outras explorações de protocolo.
	\item \textbf{Configurações avançadas:} Avaliação de recursos como \textit{Perfect Forward Secrecy} (PFS), \textit{OCSP Stapling}, e resistência a ataques de \textit{downgrade}.
\end{itemize}

A realização periódica desses testes é fundamental para assegurar que a comunicação entre cliente e servidor seja realizada de forma segura, protegendo dados sensíveis contra interceptação, modificação ou vazamento. 
Além disso, a conformidade com padrões modernos de criptografia contribui para o cumprimento de regulamentações de proteção de dados, como a Lei Geral de Proteção de Dados (LGPD) no Brasil.

A Figura~\ref{fig:ssl-test} apresenta o resultado da avaliação SSL/TLS realizada pela ferramenta SSL Labs, demonstrando a configuração de segurança implementada na aplicação:

\begin{figure}[htb]
	\centering
	\caption{Resultado do Teste de Segurança SSL/TLS}
	\includegraphics[width=\textwidth]{cap04-desenvolvimento/images/4-5-ssl-test.jpg}
	\label{fig:ssl-test}
	\fonte{Produzido pelos autores}
\end{figure}

A aplicação obteve classificação \textbf{A+}, a pontuação máxima possível na avaliação do SSL Labs. 
Essa nota representa o mais alto nível de conformidade com as melhores práticas de segurança SSL/TLS, 
indicando que o servidor está configurado com protocolos modernos, cifras robustas, certificados válidos e proteções contra vulnerabilidades conhecidas.
\FloatBarrier

\subsection{Testes automatizados}

Os testes automatizados deste projeto estão integrados ao fluxo de integração
contínua por meio da ferramenta \textbf{\textit{GitHub Actions}}~\cite{githubactions-2025}. 
A cada \textit{push} ou \textit{pull request} direcionado ao repositório principal,
uma esteira de validação é acionada com o objetivo de impedir que alterações com
erros básicos, vulnerabilidades conhecidas ou regressões evidentes sejam
incorporadas à base de código.

No contexto da \textit{CI}, são executados automaticamente 
apenas os testes unitários do \textit{back-end}. 
Os testes de integração permanecem fora da esteira, sendo realizados de forma
manual em ambiente de desenvolvimento ou homologação, em função da maior
complexidade de configuração de dependências externas (banco de dados e serviços
de terceiros). 
Para o \textit{front-end}, a validação automatizada concentra-se em análise
estática e verificação de tipos, sem execução de testes unitários automatizados.

De forma resumida, a esteira de \textit{CI/CD} contempla as seguintes etapas
principais:

\begin{itemize}
	\item \textbf{Verificação de tipos em TypeScript (\textit{front-end} e \textit{back-end}):}
	execução de uma etapa de \textit{TypeScript type check}, que compila os projetos
	sem geração de artefatos finais, garantindo que não existam inconsistências de
	tipos capazes de comprometer o processo de \textit{build}.
	
	\item \textbf{Execução de testes unitários do back-end:}
	disparo da suíte de testes unitários do \textit{back-end} com o \textit{framework}
	\textbf{Vitest}~\cite{vitest-2025}, incluindo a geração de relatórios de cobertura.
	
	\item \textbf{Análise estática e qualidade de código:}
	envio do código para inspeção pela plataforma \textbf{SonarQube/SonarCloud}~\cite{sonarqube-2025},
	complementando as verificações locais realizadas com \textbf{ESLint}~\cite{eslint-2025}. 
	Essa etapa calcula métricas de qualidade, identifica \textit{code smells},
	possíveis vulnerabilidades e duplicações, aplicando um \textit{quality gate}
	antes da aprovação do \textit{merge}. No caso do \textit{front-end}, essa
	análise estática é o principal mecanismo automatizado para detectar problemas
	que poderiam inviabilizar a \textit{build}.
	
	\item \textbf{Varredura de segurança em dependências:}
	utilização da ferramenta \textbf{Snyk}~\cite{snyk-2025} para identificar
	vulnerabilidades conhecidas em bibliotecas e dependências externas do projeto.
	Caso alguma falha crítica seja detectada, o \textit{pipeline} é marcado como
	reprovado até que a correção seja realizada.
\end{itemize}

Com essa configuração, cada solicitação de \textit{merge} para a \textit{branch} de desenvolvimento (\textit{develop}) precisa passar
obrigatoriamente pelas etapas de verificação de tipos, execução de testes
unitários do \textit{back-end}, análise estática e varredura de segurança. 
Dessa forma, a ramificação principal do repositório permanece em um estado
constantemente executável, testado e monitorado quanto à qualidade e à segurança,
tanto no \textit{back-end} quanto no \textit{front-end}.
...
A figura abaixo apresenta o relatório de cobertura dos testes unitários do \textit{back-end}. Os resultados demonstram o atendimento à meta estabelecida de 70\% para a métrica de instruções/linhas, evidenciando a eficácia da estratégia de testes adotada.

\begin{figure}[htb]
  \centering
  \caption{Resultado da Cobertura de Testes com Vitest}
	\includegraphics[width=\textwidth]{cap04-desenvolvimento/images/4-4-8-4-resultado-cobertura-teste-vitest.png}
  \label{fig:vitest-coverage-result}
  \fonte{Produzido pelos autores}
\end{figure}
\FloatBarrier

A seguir, apresenta-se a análise detalhada dos principais indicadores de cobertura:
\begin{itemize}
	\item \textbf{Instruções (Statements):} Corresponde ao percentual de declarações e comandos executados pelos testes. O valor obtido foi de 96,71\%, indicando ampla cobertura das instruções presentes no código.
	\item \textbf{Ramos (Branches):} Refere-se ao percentual de desvios condicionais (como estruturas \texttt{if/else}) que foram percorridos durante a execução dos testes. O projeto alcançou 92,32\%, demonstrando que diferentes caminhos lógicos foram adequadamente verificados.
	\item \textbf{Funções (Functions):} Indica o percentual de funções chamadas ao longo da execução dos testes. A cobertura obtida foi de 98,55\%, comprovando que praticamente todas as funções foram exercitadas.
	\item \textbf{Linhas (Lines):} Representa o percentual de linhas executáveis que foram de fato executadas. O valor alcançado foi de 96,71\%, reforçando a abrangência da cobertura.
\end{itemize}

Esses resultados evidenciam que o conjunto de testes unitários é robusto e cobre de maneira eficaz as principais estruturas do código, garantindo maior confiabilidade ao \textit{back-end} desenvolvido.